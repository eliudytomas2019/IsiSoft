...
class Saft{
    ...
    public function XML($inicio, $fim, $id){
        ...
        $this->Xml .= '<BusinessName>'.($this->Dados['empresa']).'</BusinessName>';
        ...
        $this->Xml .= '<ProductCompanyTaxID>5301035886</ProductCompanyTaxID>';
        $this->Xml .= '<SoftwareValidationNumber>293/AGT/2021</SoftwareValidationNumber>';
        $this->Xml .= '<ProductID>IsiSoft/E.IMSA LIMITADA</ProductID>';
        $this->Xml .= '<ProductVersion>Ver.1.1</ProductVersion>';
        $this->Xml .= '<HeaderComment>Software de Facturação</HeaderComment>';
        $this->Xml .= '<Telephone>923213535</Telephone>';
        $this->Xml .= '<Fax>0</Fax>';
        $this->Xml .= '<Email>e.imsalda@imsaservice.co.ao</Email>';
        $this->Xml .="</Header>";
        ...
        if($Read->getResult()):
            foreach ($Read->getResult() as $key):
                ...
                $this->Xml .= '<AddressDetail>'.($key['addressDetail']).'</AddressDetail>';
